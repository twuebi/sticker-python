...
impl PyDecoder {
    ...
    fn decode_sentences(&self, sentences: Vec<&PySentence>, labels: ListVec<ListVec<ListVec<PyLabel>>>) -> PyResult<()> {
        ...
        let _ = borrowed_sents
            .iter_mut().zip(labels.deref())
            .map(|(sent, sent_labels)| {
                let s = &mut **sent;
                let sent_labels: Vec<Vec<_>> = sent_labels.iter()
                    .map(|tok| tok.iter().map(|single|
                        single.into()).collect::<Vec<EncodingProb<'_,DependencyEncoding<RelativePOS>>>>())
                    .collect::<Vec<Vec<_>>>();
...
impl PyLabel {
    #[new]
    fn new(obj: &PyRawObject, distance: isize, pos: &str, relation: &str, probability: f32) -> PyResult<()>
    {
        obj.init(
            PyLabel {
                distance: distance,
...
impl<'a> Into<EncodingProb<'a, DependencyEncoding<RelativePOS>>> for &PyLabel {
    fn into(self) -> EncodingProb<'a, DependencyEncoding<RelativePOS>> {
        ...
        EncodingProb::new_from_owned(DependencyEncoding::new(rel_pos, self.relation.to_string()), self.probability)
    }
}
